...
% Top box B - lowered to give headroom for outside arrow
...
% Flagged space - moved 3.5mm down and to a deterministic centre placement (clears the orthogonal-join arrow)
...
% Right join J - moderate offset to stay inside page
...
% Replacement channel - moved left to stay inside margin (was at 52mm, now 38mm)
\node[block, right=10mm of B, minimum width=30mm, minimum height=12mm] (Rep) at (38mm,-6mm) {$\operatorname{Rep}(A,B)$\\$\mathcal R_{\sigma}$};
% Single arrow from B1 up and outside, then right, then down - ONE label only, above and outside
...
% Perfect separation - anchored below Rep, inside page
...
% Balanced note - reduced width to stay inside page
...
% Lower envelope beta_r: 3/2 at r=0, 1 on [1,2], 0.5 on [3,11], 0 at 12
...
% Upper envelope U_r: 1.5 on [0,3], 1 on [4,11], 0 at 12
...
% Interleaving maps - compact spacing
