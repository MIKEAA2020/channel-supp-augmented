...
# -*- coding: utf-8 -*-
"""
WAVE 13C-6 : THE PER-SHEET SEAM GAUGING (route b) + FULL BATTERY + ORBIT VERDICT.

Route (WAVE13C_L12.md section 3, option b -- the user directive 'proceed with b'):
  * The level-12 Z/3 obstruction (t_+ - t_- = t0 residue on the SHARED R-cells)
    is dissolved by the SEAM GAUGING: the sheet->R interfaces (tau-tilde) are
    per-sheet (always were), and the 5 mixed-F 'seam' walls (F5..F9) carry
    PER-SHEET PRISM SWEEPS (the seam data) instead of the hard zero-jump
    closures that v2/v4 imposed. The sweep directions are pinned by the
    c-orbit structure (c translates the F-fibres, so the drift direction is
    preserved around each F-c-orbit):
       {F1,F5,F2}: (1,0)   {F3,F7,F5'}: (0,1)   {F4,F6,F8'}: (1,-1)
    (0-based F-indices: {0,4,1}: (1,0); {2,7,5}: (0,1); {3,6,8}: (1,-1)).
    The prism set is then ALL NINE F-cells = c-invariant, which also repairs
    the prism-equivariance (the v4 model's prism walls {F1..F4} were NOT a
    c-invariant set -- T(prism on F1) landed on F5 which had no prism).
  * The v5 constraint system (all machine-exact, mod 12):
      (E1) dT=Td on the sheets, R-terms (NATURAL kappa, single T-map):
           A*tau_{cp} - B*tau_{rp} = B*t_s - A*kappa_cp,
           A = co(s,cp)*SB[cp], B = co(s,rp)  (A=B on all 12 pairs).
      (E2) seam closures: tau_{ra} + w_{ra->F} - tau_{rb} - w_{rb->F}
           = alpha_{s,F} * d_F  (the perpendicular components are HARD).
      (E3) theta-wall corners: sigma = tau_lo + w_lo;
           sigma + Delta*d = tau_hi + w_hi.
      (E6) prism-equivariance chains around the F-c-orbits:
           sigma_{cF} = sigma_F + kappa_F - t_s;  length_{cF} = length_F
           (verified consistent with (E1) by hand on both sheets -- the
           machine re-derives and certifies it inside G5).
  * Assembly: 14910 cells (6,108,1404,4752,5472,2592,576), chi = +6, natural
    free layer (rho = 0: the natural pins are c-equivariant, E4/E5 constants
    vanish), sheet layer with per-sheet tau-tilde + theta-prisms + seam
    chains. Sheet-prism signs: FSIGN[w]*(-1)^si (theta walls),
    -(-1)^si (seams) -- derived from the d^2 co-sign structure, then
    machine-certified by G2/G5.
  * Battery: G1 counts/chi; G2 d^2=0 exact; G3 H(Fl_3) Betti + torsion
    ladder; G4 T^3=I; G5 dT=Td exact; G6 freeness; G7 N=1+T+T^2=0 on H_2;
    G8 T=+1 on H_6; then the orbit complex + orbit SNF -> H_2(B_3) ->
    THE QUTRIT BIT (delta_2 = 4/3 <=> H_2(B_3) = Z/3).

Run:  python3 wave13c_seam.py
"""
import itertools
import json
import sys
...
# ============================================================================
hdr("PART III: the v5 PER-SHEET SEAM-GAUGED constraint system")
...
# The SEAM structure (the route-b gauging): the 5 mixed-F walls carry
# PER-SHEET prism sweeps. Directions from the F-c-orbits (c translates the
# F-fibres, so the drift direction is preserved around each orbit):
#   orbit {F0,F4,F1} (cells 15,19,16): all d = (1,0)
#   orbit {F2,F7,F5} (cells 17,22,20): all d = (0,1)
#   orbit {F3,F6,F8} (cells 18,21,23): all d = (1,-1)
SEAMF = [4, 5, 6, 7, 8]
SEAM_DIR = {4: (1, 0), 5: (0, 1), 6: (1, -1), 7: (0, 1), 8: (1, -1)}
MIX = [(4, 0, 3), (5, 0, 1), (6, 0, 2), (7, 3, 4), (8, 3, 5)]  # (fi, ra, rb)


def build_system():
    """The v5 system: the per-sheet seam gauging, natural free layer.

    T-map: NATURAL everywhere (kappa on R/F/E, t_s on the sheets) -- single,
    shared, exactly as pinned in stage 2a. The sheet->R interfaces
    tau_{si,R} are PER-SHEET free data; the 5 seam walls carry per-sheet
    prism sweeps alpha_{si,F} * d_F; the 4 theta walls carry the
    sigma/Delta prisms (v4 model).

    (E1) dT=Td on sheet cells, R-terms (NATURAL kappa):
         A*tau_{si,cp} - B*tau_{si,rp} = B*t_s - A*kappa_cp,
         A = co(si,cp)*SB[cp], B = co(si,rp), cp = c^-1(rp).
    (E2) seam closures: tau_{si,ra} + w(ra->F) - tau_{si,rb} - w(rb->F)
         = alpha_{si,F} * d_F[c]  (per component; the perpendicular
         components are HARD equalities).
    (E3) theta-wall corners: sigma = tau_lo + w(lo->F);
         sigma + Delta*d = tau_hi + w(hi->F).
    """
    equations = []
    VARS = set()
    ...
    # (E1) -- R-term equivariance, natural kappa, per sheet
    for si in (0, 1):
        ...
        for rp in range(6):
            ...
            B = co(si, rp)
            for c in range(2):
                eq_add({tvar(si, cp, c): A, tvar(si, rp, c): -B},
                       (B * t[c] - A * KAPPA_R[cp][c]))
    # (E2) -- seam closures with per-sheet sweeps
    for si in (0, 1):
        for (fi, ra, rb) in MIX:
            ...
            for c in range(2):
                ...
                if d[c]:
                    lhs[avar(si, fi)] = -d[c]
                eq_add(lhs, (wb[c] - wa[c]))
    # (E3) -- theta-wall corners
    for si in (0, 1):
        for w in WALLS:
            lo, hi = RLO[w], RHI[w]
            d = WDIR[w]
            ...
            for c in range(2):
                eq_add({svar(si, w, c): 1, tvar(si, lo, c): -1}, wlo[c])
...
print("v5 system: %d equations, %d variables" % (len(eqs), len(VLIST)))
with open("wave13c_seam_eqs.json", "w") as f:
    json.dump({
...
sol = solve_mod12(eqs)
if sol is None:
    hdr("THE V5 SEAM SYSTEM IS INCONSISTENT (honest stop)")
    print("The per-sheet seam-gauged system (E1 natural + E2 seam closures")
    print("+ E3 theta corners) has no mod-12 solution. The level-12 mod-3")
    print("obstruction is NOT dissolved by this formulation. Report and stop.")
    sys.exit(1)
print("v5 system SOLVED mod 12 (mod-3 + Hensel mod-4 + CRT); verified on all %d equations: PASS"
      % len(eqs))
bad = 0
for (lhs, rhs) in eqs:
    val = sum(cc * sol[v] for v, cc in lhs.items()) % MOD
    assert val == rhs % MOD, "v5 solution wrong"
...
ALP = {si: {fi: sol[('a', si, fi)] for fi in SEAMF} for si in (0, 1)}
for si in (0, 1):
    sn = "s+" if si == 0 else "s-"
    print("  tau-tilde (%s): %s" % (sn, {RNAME[r]: TT[si][r] for r in range(6)}))
    print("  sigma (%s): %s  Delta (%s): %s" % (sn, SIG[si], sn, DEL[si]))
    print("  seam alphas (%s): %s" % (sn, ALP[si]))
tick("Part III done")
...
def h_unit_u(fc):
    """Unit-u translation homotopy h_u on the T2 cellulation, exact:
      d(h_u s) + h_u(d s) = tau_u(s) - s   for every cell s.
    Derived cell-by-cell (P -> +H(j,i); H -> 0 (parallel); V -> +(L+U);
    A -> -[U(k,c-k-1)+L(k+1,c-k-1)]; L,U -> 0 -- the identity then holds
    on L/U via h(d)). Machine-certified below by h_cert()."""
    k = fc[0]
    if k == 'P':
        return [(1, ('H', fc[2] % MOD, fc[1] % MOD))]
    if k == 'V':
        return [(1, ('L', fc[1] % MOD, fc[2] % MOD)),
                (1, ('U', fc[1] % MOD, fc[2] % MOD))]
    if k == 'A':
        c, kk = fc[1] % MOD, fc[2] % MOD
        return [(-1, ('U', kk, (c - kk - 1) % MOD)),
                (-1, ('L', (kk + 1) % MOD, (c - kk - 1) % MOD))]
    return []


def h_unit_v(fc):
    """Unit-v translation homotopy h_v (exact, same identity):
      P -> +V(i,j); V -> 0 (parallel); H(j,i) -> -(L+U)(i,j);
      A -> -[L(k,c-k) + U(k,c-k-1)]; L,U -> 0."""
    k = fc[0]
    if k == 'P':
        return [(1, ('V', fc[1] % MOD, fc[2] % MOD))]
    if k == 'H':
        return [(-1, ('L', fc[2] % MOD, fc[1] % MOD)),
                (-1, ('U', fc[2] % MOD, fc[1] % MOD))]
    if k == 'A':
        c, kk = fc[1] % MOD, fc[2] % MOD
        return [(-1, ('L', kk, (c - kk) % MOD)),
                (-1, ('U', kk, (c - kk - 1) % MOD))]
    return []


def h_mu(mu, fc):
    """Translation homotopy h_mu for mu = (muu, muv) twelfths, exact:
      d h_mu(s) + h_mu(d s) = tau_mu(s) - s.
    Composite: h_mu = h_{(muu,0)} + tau_{(muu,0)} h_{(0,muv)};
    multi-step: sum of unit homotopies at successive translates;
    negative components via the reversed telescoping (h_{-u} = -tau_{-u} h_u)."""
    ...
    if muu >= 0:
        for m in range(muu):
            for (co, c2) in h_unit_u(fc):
                out.append((co, t2_tr(c2, m, 0)))
    else:
    ...
    if muv >= 0:
        for n in range(muv):
            for (co, c2) in h_unit_v(fc):
                out.append((co, t2_tr(c2, muu, n)))
    else:
...
def h_cert():
    """Machine certificate: d(h s) + h(d s) = tau(s) - s exactly for the
    unit homotopies on every cell type."""
    for fc0 in [('P', 3, 7), ('H', 2, 5), ('V', 5, 1), ('A', 4, 2),
                ('L', 6, 3), ('U', 1, 8)]:
        for (h, d1, d2, tag) in [(h_unit_u, 1, 0, 'u'), (h_unit_v, 0, 1, 'v')]:
            lhs = {}
            for (co, c2) in h(fc0):
                for (co2, c3) in t2_bound(c2):
                    lhs[c3] = lhs.get(c3, 0) + co * co2
            for (co, c2) in t2_bound(fc0):
                for (co2, c3) in h(c2):
                    lhs[c3] = lhs.get(c3, 0) + co * co2
            lhs = {k2: v for k2, v in lhs.items() if v != 0}
            rhs = {t2_tr(fc0, d1, d2): 1, fc0: -1}
            assert lhs == rhs, \
                "homotopy identity FAILS at %s dir %s:\n  got %s\n  want %s" % (
                    fc0, tag, lhs, rhs)
    print("homotopy identity d(h s)+h(d s)=tau(s)-s: EXACT on all cell types (u,v): OK")


h_cert()


def seam_theta_sign(si, ri, fi):
    """The prism-sign of the F-wall for sheet si: +co(s,R_start)*co(R_start,F)
    (theta walls: start = lo; seams: the sweep runs rb -> ra with sign
    -co(s,R_ra)*co(R_ra,F)). Derived from the d^2 co-sign structure."""
    return D_SHEET[si][24 + ri] * D_REG[ri][15 + fi]


FSIGN = {w: seam_theta_sign(0, RLO[w], WFI[w]) for w in WALLS}
...
def sheet_col(si, fc):
    """Boundary of the sheet cell (s_si, fc): R-terms (per-sheet tau-tilde),
    theta-wall prisms + SEAM chains = the translation homotopies h_mu
    (exact, certified by h_cert), fibre Leibniz.
    Theta wall w: sweep lo->hi: h_{Delta*d}(fc + sigma), sigma = tau_lo+w_lo,
    sign = +co(s,R_lo)*co(R_lo,F_w) * sheet-mult (-1)^si.
    Seam F: sweep rb->ra: h_{alpha*d}(fc + tau_rb + w_rb),
    sign = -co(s,R_ra)*co(R_ra,F) * sheet-mult... derived, then
    machine-certified by the exact G2 check."""
    out = []
    for ri in range(6):
        co = +1 if ri >= 3 else -1
        if si == 1:
    ...
    for (fi, ra, rb) in MIX:
        wa = W_RF.get((ra, fi), (0, 0))
        wb = W_RF.get((rb, fi), (0, 0))
        # SEAM SWEEP CONVENTIONS (pinned by the c-orbit equivariance E6):
        # the c-edges F0->F4->F1 (preserve), F3->F6 (preserve), F2->F7,
        # F7->F5, F6->F8, F8->F3 (swap the R-parent roles). The seams on the
        # swapped edges (F7, F8) sweep ra->rb (start = ra-path, mu = rb-ra,
        # sign +co_a); the others sweep rb->ra (start = rb-path, mu = ra-rb,
        # sign -co_a). Machine-certified by the G5 check.
        if fi in (7, 8):
...
# sign-variant trials: the per-sheet global flip eps_s on (theta, seam) signs
# (seam sign = -eps_s*flip... derived signs above are the default; the
# machine picks by the exact d^2=0 certificate)
BEST = None
for flip in (1, -1):
    D = dict(DFREE_NAT)
    for si in (0, 1):
    ...
    tag = "seam assembly (signs as derived)"
    if bad == 0:
        print("  %-46s: sheet layer d^2 = 0 EXACTLY: PASS" % tag)
        BEST = (tag, D)
        break
    print("  %-46s: d^2 residuals %d (e.g. %s)" % (tag, bad, first[:3]))
    break
if BEST is None:
    hdr("SEAM ASSEMBLY FAILED (d^2 != 0)")
    print("The v5-solved sheet data does not close d^2=0 with the seam-chain")
    print("model. Honest report; the witnesses above pin the defect.")
    print("delta_2(qutrit) REMAINS OPEN.")
    sys.exit(1)
(tag, D) = BEST
print("SELECTED: %s" % tag)
...
print("  seam alphas (s+): %s" % ALP[0])
print("  seam alphas (s-): %s" % ALP[1])
...
def coker_order9(k):
    ...
    while changed:
        ...
        for j in range(n):
            ...
            M[r] = (M[r] * inv) % 9
            col = M[r + 1:, j]
            # eliminate ALL nonzero below-entries (units via the inverse
            # multiple, 3-entries via the 3-multiple) -- the v4/l12 version
            # skipped the 3-entries (col % 3 == 0), leaving nonzero entries
            # in the pivot columns of the eventual free rows, which then
            # polluted the L-residual through the column-clear step and
            # inflated the 3-rank (t=36, inconsistent with d^2=0 mod 9).
            nzi = np.nonzero(col)[0]
            if len(nzi):
                rows = r + 1 + nzi
