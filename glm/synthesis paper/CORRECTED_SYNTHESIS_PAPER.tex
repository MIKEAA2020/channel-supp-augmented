...
\title{\bfseries Exact Affine Geometry, Central Radii, and Certified Compression Bounds for Finite-Outcome Quantum Instruments}
\author{Anonymous}
\date{}
...
\]
with $\mathcal I_*$ an optimal centre and the diameter of the body exactly $2$. No centre of the body carries a larger relative ball: the global in-radius, i.e.\ the supremum over all centres, equals the centred value, so $\mathcal I_*$ is an optimal in-radius centre as well. The replacement face has exact antipodal profile
\[
...
The main contribution of this paper is the exact affine geometry of the instrument body and its compression consequences; among the exact parameters, the uniform depolarising instrument is an optimal centre for both the covering radius and the global in-radius. Which quantities are determined \emph{exactly}, and which only by \emph{certified bounds}, is stated once in Remark~\ref{rem:exact-vs-certified} and maintained throughout. A separate physical question --- whether an abstract code can be realised by a fixed programmable processor --- is addressed in Appendix~\ref{app:no-programming}: exact finite-dimensional evaluation of all unitary channels is impossible, and vanishing error forces unbounded program dimension. This separates \emph{representational} dimension from \emph{programmable} evaluation; the main theorems do not rely on the appendix.
...
The following statements are exact: (i) the affine dimension $D_{A,B}^{(n)}$ (Proposition~\ref{prop:dimension}); (ii) the centred in-radius at the uniform instrument (Theorem~\ref{thm:exact-radius}); (iii) the global covering radius and optimality of $\mathcal I_*$ as a centre (Theorem~\ref{thm:covering}); (iv) the antipodal profile of the replacement face (Theorem~\ref{thm:replacement-profile}), which is also the full-body profile when $d_A=1$; (v) the zero-error Euclidean threshold for continuous encoding with unrestricted decoding (Theorem~\ref{thm:zero-error}); (vi) the diameter (Proposition~\ref{prop:diameter}); (vii) the constant-code width $\delta_{0,\mathrm{inst}}^{\diamond}(A,B)$ (Corollary~\ref{cor:constant-code}); (viii) the top-subcritical replacement width (Corollary~\ref{cor:top-subcritical}); and (ix) the complete width function in the collapse case $nd_B\min\{d_A,d_B\}=2$ (Theorem~\ref{thm:collapse}); and (x) the global in-radius over all centres, with $\mathcal I_*$ an optimal in-radius centre (Theorem~\ref{thm:global-inradius}). The following are certified bounds or open quantities, not exact determinations: (a) the intermediate compression widths $\delta_{r,\mathrm{inst}}^{\diamond}(A,B)$ for $0<r<D_{A,B}^{(n)}$ outside the cases (vii)--(ix); (b) the full antipodal profile beyond the certified constructions; (c) optimality of the certified upper envelope $U_r^{(n)}$.
\end{remark}
...
\begin{proof}
If $\lambda_{\max}(-X)\le0$ there is nothing to prove. Otherwise, since $X$ is block diagonal, $\lambda_{\max}(-X)=\max_k\lambda_{\max}(-X_k)$. Choose $k_0$ attaining the maximum and a unit vector $|v\rangle\in A_0\otimes B$ with $X_{k_0}|v\rangle=-\lambda|v\rangle$, $\lambda=\lambda_{\max}(-X)$. Let $W\subseteq A_0$ be the support of $\Tr_B|v\rangle\langle v|$ and put $b=\dim W$: the rank of $\Tr_B|v\rangle\langle v|$ is the Schmidt rank of $|v\rangle$ across $A_0\otimes B$, so $b\le\min\{d_A,d_B\}=s$. If $|e_1\rangle,\dots,|e_b\rangle$ is an orthonormal basis of $W$ and $|\overline e_j\rangle\in A$ its coordinatewise conjugate under the fixed Choi bases, define
\[
...
Theorem~\ref{thm:exact-radius} determines the largest ball centred at $\mathcal I_*$; whether some other centre of $\Inst_n(A,B)$ could carry a larger relative ball --- which would improve the intermediate certified bounds below --- is settled in Theorem~\ref{thm:global-inradius} below: no centre does, and the global in-radius
\[
\sup_{\mathcal I_0\in\Inst_n(A,B)}\rho_{\diamond,\mathrm{inst}}(\mathcal I_0)
\]
equals the centred value $2/(nd_B\min\{d_A,d_B\})$.
\end{remark}
...
Write
\[
B_{\mathfrak V}(\rho)=\{\vD\in\mathfrak V_{A,B}^{(n)}:\|\vD\|_{\diamond,\mathrm{inst}}\le\rho\}
\]
for the closed direction ball. For centres $\mathcal I_0,\mathcal I_1\in\Inst_n(A,B)$ and $\lambda\in[0,1]$,
\[
...
\begin{proof}
For centrally symmetric convex balls one has the Minkowski identity
\[
\lambda B_{\mathfrak V}(\rho_0)+(1-\lambda)B_{\mathfrak V}(\rho_1)
=B_{\mathfrak V}\big(\lambda\rho_0+(1-\lambda)\rho_1\big):
\]
the inclusion ``$\subseteq$'' is the triangle inequality, and ``$\supseteq$'' follows from the decomposition $\rho u=\lambda\rho_0u+(1-\lambda)\rho_1u$ with $\rho=\lambda\rho_0+(1-\lambda)\rho_1$ and $\|u\|_{\diamond,\mathrm{inst}}\le1$. Assume $\mathcal I_i+B_{\mathfrak V}(\rho_i)\subseteq\Inst_n(A,B)$; every point of $\lambda\mathcal I_0+(1-\lambda)\mathcal I_1+\lambda B_{\mathfrak V}(\rho_0)+(1-\lambda)B_{\mathfrak V}(\rho_1)$ is a convex combination of two instruments, hence an instrument. Optimising $\rho_0$ and $\rho_1$ independently gives the concavity.
...
Four distinct radius-like quantities are used throughout this paper, always written with their qualifiers: (i) the \emph{centred relative in-radius} $\rho_{\diamond,\mathrm{inst}}(\mathcal I_0)$, the largest relative ball about a fixed centre that is contained in the body; (ii) the \emph{worst-case covering radius} $R_{\mathrm{inst}}(\Psi)$, the maximal distance from a centre to the body; (iii) the \emph{antipodal profile} $\alpha_r$, a width-like sequence indexed by the sphere dimension $r$, which is not the radius of any ball; and (iv) the \emph{global in-radius}, the supremum of the centred in-radius over all centres of the body (Theorem~\ref{thm:global-inradius}). The word ``radius'' without a qualifier is avoided.
\end{remark}
...
\begin{proof}
Let $h:S^{r+1}\to K$ be continuous and restrict it to an equatorial copy $S^r\simeq\{x\in S^{r+1}:x_{r+1}=0\}$. The antipodal map on the equator is the ordinary antipodal map on $S^r$, hence
\[
...
\]
The set $\RepInst_n(A,B)$ is a face of $\Inst_n(A,B)$: if $\lambda\vec{\mathcal R}_\tau+(1-\lambda)\mathcal J\in\RepInst_n(A,B)$ with $\mathcal J\in\Inst_n(A,B)$ and $0<\lambda<1$, write $\lambda\vec{\mathcal R}_\tau+(1-\lambda)\mathcal J=\vec{\mathcal R}_\sigma$; then each component of $\mathcal J$ satisfies $\mathcal J_k(X)=\Tr(X)\tilde\tau_k$ with $\tilde\tau_k=(\sigma_k-\lambda\tau_k)/(1-\lambda)$, complete positivity of $\mathcal J$ forces $\tilde\tau_k\ge0$, and the summed trace condition gives $\sum_k\Tr\tilde\tau_k=1$, so $\mathcal J=\vec{\mathcal R}_{\tilde\tau}\in\RepInst_n(A,B)$. This justifies the name \emph{replacement face} used below.
...
\begin{proof}
First suppose $n\ge2$. Fix a unit vector $|b_0\rangle\in B$ and let $\sigma=|b_0\rangle\langle b_0|$. Write $\Herm_0(A)$ for the real vector space of traceless Hermitian operators on $A$, of real dimension $d_A^2-1$. For $H\in\Herm_0(A)$, $H\neq0$, define
\[
...
Proposition~\ref{prop:join-inst} joins one \emph{input-dependent} sphere with arbitrarily many replacement spheres: the replacement components attain their maximal separation $2$ on \emph{every} input, so the eigenvector input that exposes the input-dependent component simultaneously exposes all of them. Two input-dependent components could not be joined by this mechanism, since their separating inputs (eigenvectors of the respective observables) vary with the sphere parameter and need not coincide; this is why exactly one observable sphere enters Corollary~\ref{cor:join-dimension} below.
\end{remark}
...
\]
in the case $n\ge2$ (with the pure state $\sigma=|b_0\rangle\langle b_0|$), and on the $2$-dimensional flagged subspace $W_{\mathrm{obs}}=\operatorname{span}\{\sigma_+,\sigma_-\}$ in the case $n=1$. Let $W_{\mathrm{obs}}^{\perp}$ denote the per-block orthogonal complement of $W_{\mathrm{obs}}$: its first two blocks are $\{|b_0\rangle\}^{\perp}\subseteq B$ and its remaining $n-2$ blocks are all of $B$ when $n\ge2$, and it is the orthogonal complement of $W_{\mathrm{obs}}$ in $B$ when $n=1$. The real vector space of flag-block-diagonal Hermitian operators supported on $W_{\mathrm{obs}}^{\perp}$ has dimension
\[
N_{\mathrm{join}}=
\begin{cases}
...
\end{cases}
\]
When $L_{A,B}^{(n)}=D_{A,B}^{(n)}-1$ the middle range is empty (this is always the case for $d_A=1$). Every component of the envelope is rigorously certified: the value $1$ on the first range by Theorems~\ref{thm:replacement-profile} and \ref{thm:observable} and Corollary~\ref{cor:join-dimension}, the value $\gamma_{A,B}^{(n)}$ on the middle range by Theorems~\ref{thm:ball} and \ref{thm:full-direction}, and the value $0$ on the last range by Theorem~\ref{thm:zero-error}. Within the ball mechanism the term $\rho_n$ is optimal over all centres of the body (Theorem~\ref{thm:global-inradius}).
\end{definition}
...
=\delta_{r,\mathrm{inst}}^{\diamond}(A,B),\\
\delta_{r,\mathrm{inst}}^{\diamond}(A,B)\le U_r^{(n)}(A,B).
\end{gather*}
...
\]
with the convention $\coind\emptyset=-1$ and the value $+\infty$ also allowed. The deleted product carries the subspace topology of $Y^2$ and the free $\mathbb Z_2$-action exchanging the entries; since $Y$ is Hausdorff, the diagonal of $Y^2$ is closed and $F_2(Y)$ is its open complement. Only the existence of equivariant maps into $F_2(Y)$ is used below. If $\coind F_2(Y)=+\infty$, no integer $q$ satisfies $q>\coind F_2(Y)$ and the corollary below is vacuous. If $Y$ admits a continuous injection $\jmath:Y\to\mathbb R^r$, the normalised difference map
\[
...
\]
on an auxiliary Euclidean sphere $S^q\subset W$ (the denominator is nonzero on the Euclidean unit sphere, and finite-dimensional norm equivalence makes the normalisation continuous). Theorem~\ref{thm:ball} makes this map instrument-valued, and its antipodal separation is $2\rho_n$; alternatively, the full-direction sphere of Theorem~\ref{thm:full-direction}, restricted to an equatorial $S^q$, has antipodal separation $2/d_A$. In both regimes, if the two latent codes $\operatorname{Enc}(h(u))$ and $\operatorname{Enc}(h(-u))$ were distinct for every $u$, then $u\mapsto(\operatorname{Enc}(h(u)),\operatorname{Enc}(h(-u)))$ would be an equivariant map $S^q\to F_2(Y)$, contradicting the coindex hypothesis. An antipodal pair therefore has one code; the decoder returns one reconstruction for it, and the triangle inequality gives half the antipodal separation of the sphere used. In the first regime the half-separation is $1=\beta_q^{\mathrm{best},(n)}(A,B)$. In the second regime the radial sphere and the equatorial restriction of the full-direction sphere, used separately, give error at least $\rho_n$ and at least $1/d_A$, hence at least $\beta_q^{\mathrm{best},(n)}(A,B)=\gamma_{A,B}^{(n)}=\max\{\rho_n,1/d_A\}$.
\end{proof}
...
\label{prop:gap}
Let $m_n=nd_B^2-1$, $D_n=D_{A,B}^{(n)}$ (Proposition~\ref{prop:dimension}), $\rho_n=2/(nd_Bs)$, $R_n=2-\rho_n$. Then:
\begin{enumerate}[leftmargin=*,itemsep=2pt]
...
\item For $L_{A,B}^{(n)}<r<D_n$, $\beta_r^{\mathrm{best},(n)}=\gamma_{A,B}^{(n)}$ and $U_r^{(n)}\ge1$, so the \emph{certified} bracket has length at least $1-\gamma_{A,B}^{(n)}$; this bounds the certificate, not the unknown true width, and the ball contribution $\rho_n$ to $\gamma_{A,B}^{(n)}$ is optimal over all centres (Theorem~\ref{thm:global-inradius}).
...
\begin{proof}
Item 1: at $r=0$ no pinching term is admissible below $D_n$, because a pinching family with at least two blocks has threshold at least $d_A^2(n\cdot2-1)\ge d_A^2\ge1$ for output pinching and at least $2(nd_B^2-1)\ge2$ for input pinching, while a single block gives the trivial threshold $D_n$ itself; hence $U_0$ is the minimum of $R_n$ and $2D_n/(D_n+1)$, and
\[
2-\frac{2}{D_n+1}\ge 2-\frac{2}{nd_Bs}=R_n
\]
because $D_n+1=d_A^2(nd_B^2-1)+1\ge nd_B^2\ge nd_Bs$; hence $U_0=R_n$, and with $\beta_0^{\mathrm{best},(n)}=1$ the gap is exactly $R_n-1=1-\rho_n$. Items 2--6 restate the bracket (Corollary~\ref{cor:bracket}), the floor (Proposition~\ref{prop:floor}), Corollaries~\ref{cor:constant-code} and \ref{cor:top-subcritical}, and Theorems~\ref{thm:zero-error} and \ref{thm:collapse}; compare also Corollary~\ref{cor:cutoff} for the antipodal profile at and above the affine dimension.
\end{proof}
...
Three candidate tools, however, do \emph{not} break the barrier, and it is worth recording why. (i) \emph{Covering-number encodings.} An $\varepsilon$-net of a $D$-dimensional convex body has size $N(\varepsilon)\ge(r_0/\varepsilon)^{D}$ up to constants, where $r_0$ is the radius of any ball inscribed in the body (which exists by full affine dimension, and transfers between equivalent norms), so a net-based code needs $\Theta(D\log_2(1/\varepsilon))$ bits --- more than $D$ bits for every $\varepsilon<r_0/2$. The affine encoding already achieves error $0$ with exactly $D$ coordinates (Theorem~\ref{thm:zero-error}); nets are therefore strictly dominated by affine coordinates and cannot certify sub-one error below the affine threshold. (ii) \emph{Orthogonal joins of separation-two spheres.} These are precisely the constructions already absorbed into $\beta_r^{\mathrm{best},(n)}$ in Section~\ref{sec:lower}: replacement-only joins are maximised by the single-block partition (Remark~\ref{rem:partitions}, giving $L_{\mathrm{rep}}$), and the join mechanism of Proposition~\ref{prop:join-inst} admits at most one input-dependent sphere (Remark~\ref{rem:join-one-observable}, giving $L_{\mathrm{join}}$). (iii) \emph{Re-centred balls.} The intermediate certificate $\rho_n$ is realised by the radial construction about $\mathcal I_*$ (Theorem~\ref{thm:ball}); by Theorem~\ref{thm:global-inradius} no centre of the instrument body carries a larger relative ball, so re-centring cannot push the ball mechanism past $\rho_n$. Within mechanisms (i) and (ii) the value $1$ is certified up to $L_{A,B}^{(n)}$ and not beyond; whether some other construction extends the separation-two regime past $L_{A,B}^{(n)}$ is open (Remark~\ref{rem:exact-vs-certified}(b)), and by (iii) any improvement of the intermediate lower bound $\gamma_{A,B}^{(n)}=\max\{\rho_n,1/d_A\}$ must come from mechanisms other than inscribed balls.
\end{remark}

\section{Specialisations, examples, and explicit trade-off}
\label{sec:specialisations}

Table~\ref{tab:comparison} summarises how the exact instrument parameters of the preceding sections specialise to channels and POVMs.
...
\]
with covering radius $2(1-1/(d_B\min\{d_A,d_B\}))$, global in-radius equal to the centred value by Theorem~\ref{thm:global-inradius}, and zero-error threshold $d_A^2(d_B^2-1)$. (All normalisations are the ones fixed in Remark~\ref{rem:choi-convention}; no comparison with external conventions is asserted.)

\item \textbf{POVM case $d_B=1$, $n\ge2$.} Every instrument has the form $\mathcal I_k(X)=\Tr(E_kX)$ with $E_k\ge0$, $\sum_kE_k=I_A$ (Remark~\ref{rem:povm}). The centred in-radius is $\rho_{\diamond,\mathrm{inst}}(\mathcal I_*)=2/n$, and this is also the global in-radius over all centres (Theorem~\ref{thm:global-inradius}); the flagged replacement sphere has dimension $n-2$, and the affine dimension is $d_A^2(n-1)$.
...
\end{cases}
\]
The centred in-radius is still $2/(nd_B)$ (since $s=\min\{1,d_B\}=1$) as a radius of the body inside its affine hull, and by Theorem~\ref{thm:global-inradius} it is also the global in-radius over all centres; it plays no role in the antipodal profile, which is governed by the lower-dimensional replacement structure of the body.
...
Global in-radius & $2/d_Bs$ & $2/nd_Bs$ & in-radius concave in the centre (Lemma~\ref{lem:inradius-concave}); Haar barycentre of $\mathrm U(B)\times S_n$ orbits is $\mathcal I_*$; Jensen (Theorem~\ref{thm:global-inradius})\\
...
\]
so that $\Lambda_j(W_j\pi W_j^\dagger)=\pi$ for $\pi\in\Sstate(M_j)$. The extended processors $\widetilde\Gamma_j=\Gamma_j\circ(\Lambda_j\otimes\id_E):\Lin(M\otimes E)\to\Lin(E)$ are CPTP and preserve every originally programmed channel. The set of CPTP maps on the fixed spaces $M\otimes E\to E$ is compact, so, passing to a further subsequence \emph{chosen independently of the target unitary}, $\widetilde\Gamma_j\to\Gamma$ in diamond norm. Now fix a unitary $U$. For each $j$ choose $\pi'_j(U)\in\Sstate(M_j)$ with approximation error at most $\varepsilon_j$ (such a minimiser exists by compactness of $\Sstate(M_j)$ and continuity of the induced channel), and embed it as $\pi_j(U)=W_j\pi'_j(U)W_j^\dagger\in\Sstate(M)$. By compactness of $\Sstate(M)$ there is a $U$-dependent subsubsequence with $\pi_j(U)\to\pi_U$; along it the processors still converge to the same $\Gamma$. Continuity of the evaluation map in both processor and program state gives $\Gamma(\pi_U\otimes\cdot)=\mathcal U_U$. Since $U$ was arbitrary, the single limiting processor $\Gamma$ exactly evaluates every unitary channel, contradicting Theorem~\ref{thm:no-programming}. Hence no bounded subsequence of $(m_j)$ exists, and since $(m_j)$ is integer-valued this forces $m_j\to\infty$.
\end{proof}
