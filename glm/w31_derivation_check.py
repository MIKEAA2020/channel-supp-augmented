#!/usr/bin/env python3
"""
WAVE 31: the wave-29 verifier STRENGTHENED with the lattice-index gate G3L.
(Wave 30, the referee read of Proposition prop:cupsquare, found R1: G3's rank
comparison is blind to a finite-index sublattice, and the v10 manuscript
asserted the integral module equality Z[sigma] (+) Delta.Z[sigma] = free part,
which is false integrally from degree 3 on: the true free part is the
orbit-sum invariant lattice, and in H^6(B;Z) = Z the discriminant is TWICE a
generator.  This strengthened verifier re-runs the full wave-29 battery with
the lattice fine structure now certified by G3L, matching the corrected v11
...
with H^*(BK;Z) computed by the collapsed Serre SS of BT^3 -> BK -> BC_3:
    ...
    free part  rationally  Q[sigma_1,sigma_2,sigma_3] (+) Delta.Q[sigma_1,
               sigma_2,sigma_3]; integrally the orbit-sum invariant lattice
               M_d (G3L), of which the module Z[sigma] (+) Delta.Z[sigma]
               is a finite-index sublattice (index 1 for d <= 2; 2, 2, 4, 8
               for d = 3..6)
    torsion    tau^k . (fixed-monomial detection mod 3)
    H^6(B;Z) = Z . <[S1]>  with  [Delta] = 2 [S1]   (G3L, the corrected top class)
...
G3L (wave 31) certifies the lattice-index layer: the orbit-sum lattice, the
module index per degree, the degree-3 witness S1, and the top-class quotient
H^6(B) = M_3/<s1^3, s1 s2, s3> = Z.<[S1]> with [Delta] = 2[S1].
"""
...
# ------------------------- G3L (wave 31): the lattice-index layer of G3
log("\n== G3L (wave 31): lattice-index layer -- the orbit-sum lattice vs the "
    "module ==")
log("  (root cause of referee finding R1: G3 compares RANKS only; an")
log("   index-2 sublattice is invisible to a rank check.  Here the integral")
log("   fine structure is computed exactly: M_d = the orbit-sum invariant")
log("   lattice, L_d = the module Z[sigma]_d + Delta.Z[sigma]_{d-3}.)")
G3L = True
...
def monoms_d(d):
    return sorted(m for m in itertools.product(range(d + 1), repeat=3)
                  if sum(m) == d)

def vec_of_poly(p, d):
    """Coordinates of the chi-polynomial p in the degree-d monomial basis."""
    idx = {m: i for i, m in enumerate(monoms_d(d))}
    v = [0] * len(monoms_d(d))
    for m, c in p.items():
        v[idx[m]] = c
    return v
...
def sig_mono(i, j, k):
    """sigma_1^i sigma_2^j sigma_3^k as a chi-polynomial (degree i+2j+3k)."""
    return pmul(pmul(ppow(SIG1, i), ppow(SIG2, j)), ppow(SIG3, k))

def module_gens(d):
    """Z-generators of the degree-d part of Z[sigma] (+) Delta Z[sigma]."""
    gens = []
    for i in range(d + 1):
        for j in range((d - i) // 2 + 1):
            r = d - i - 2 * j
            if r >= 0 and r % 3 == 0:
                gens.append(sig_mono(i, j, r // 3))
    for i in range(d - 2):
        for j in range((d - 3 - i) // 2 + 1):
            r = (d - 3) - i - 2 * j
            if r >= 0 and r % 3 == 0:
                gens.append(pmul(DELTA, sig_mono(i, j, r // 3)))
    return gens
...
def lattice_index(Acols, Bcols):
    """Index [lattice(Bcols) : lattice(Acols)] (columns = generators);
    None if lattice(Acols) is not contained in lattice(Bcols)."""
    X = Bcols.solve(Acols)          # rational;  A = B . X
    if not all(e == int(e) for e in X):
        return None
    Sn = smith_normal_form(S.Matrix(X), domain=ZZ)
    idx = 1
    for i in range(min(Sn.shape)):
        v = Sn[i, i]
        if v != 0:
            idx *= abs(int(v))
    return idx

def in_rowlattice(v, rows):
    """Is v in the integer row-lattice of rows (rows of full row rank)?"""
    R = S.Matrix(rows)
    assert R.rank() == R.rows, "in_rowlattice: rows not full row rank"
    vcol = S.Matrix([list(v)]).T
    try:
        sol, params = R.T.gauss_jordan_solve(vcol)
    except Exception:
        return False                # inconsistent over Q
    if params:
        return False                # cannot happen at full row rank
    return all(e.is_Integer for e in sol)

# (a) the index table, degree by degree --------------------------------
EXPECT_IDX = {0: 1, 1: 1, 2: 1, 3: 2, 4: 2, 5: 4, 6: 8, 7: 16, 8: 32}
for d in range(0, 9):
    orbits, fixed = orbit_data[d]
    Bm = S.Matrix([vec_of_poly(orbit_sum_poly(o), d) for o in orbits]).T
    Lm = S.Matrix([vec_of_poly(g, d) for g in module_gens(d)]).T
    idx = lattice_index(Lm, Bm)
    ok = (idx == EXPECT_IDX[d])
    G3L &= ok
    log(f"  degree {d}: [M_d : module] = {idx} (expected {EXPECT_IDX[d]}) "
        f"{'OK' if ok else 'MISMATCH'}")
    if d <= 2:
        G3L &= (idx == 1)
log("  -> the module is CONTAINED in the orbit-sum lattice (integral), the")
log("     two coincide rationally (finite index), and the index is ONE in")
log("     degrees 0,1,2 -- exactly the degrees of the Step-5 crux, so the")
log("     H^2/H^4 displays Z sigma_1 and Z sigma_1^2 (+) Z sigma_2 are exact")
log("     lattice statements (the referee's 'safe degrees').")

# (b) the degree-3 orbit sums and their identities ----------------------
S1P = padd(padd(mono(2, 1, 0), mono(1, 0, 2)), mono(0, 2, 1))
S2P = padd(padd(mono(2, 0, 1), mono(1, 2, 0)), mono(0, 1, 2))
A1P = padd(padd(mono(3, 0, 0), mono(0, 3, 0)), mono(0, 0, 3))
ok = (padd(padd(S1P, S2P), pscale(SIG3, 3)) == pmul(SIG1, SIG2))
G3L &= ok
log(f"  S1 + S2 + 3 sigma_3 = sigma_1 sigma_2 : {'OK' if ok else 'MISMATCH'}")
ok = (padd(S1P, pscale(S2P, -1)) == DELTA)
G3L &= ok
log(f"  S1 - S2 = Delta : {'OK' if ok else 'MISMATCH'}")
ok = (pscale(S1P, 2) == padd(padd(pmul(SIG1, SIG2), pscale(SIG3, -3)), DELTA))
G3L &= ok
log(f"  2 S1 = sigma_1 sigma_2 - 3 sigma_3 + Delta : "
    f"{'OK' if ok else 'MISMATCH'}")
ok = (padd(padd(A1P, pscale(padd(S1P, S2P), 3)),
           pscale(SIG3, 6)) == ppow(SIG1, 3))
G3L &= ok
log(f"  sigma_1^3 = A1 + 3(S1 + S2) + 6 sigma_3 : "
    f"{'OK' if ok else 'MISMATCH'}")

# (c) the degree-3 witness: S1 NOT in the module, 2 S1 in it ------------
#     (named system in the referee's order: a sigma_1^3 + b sigma_1 sigma_2
#      + c sigma_3 + e Delta; the unique rational solution forces b = e = 1/2)
NAMED = [ppow(SIG1, 3), pmul(SIG1, SIG2), SIG3, DELTA]
L3 = S.Matrix([vec_of_poly(g, 3) for g in NAMED]).T
vS1 = S.Matrix([vec_of_poly(S1P, 3)]).T
xs = L3.solve(vS1)
ok_int = all(e == int(e) for e in xs)
ok = (not ok_int)                     # the witness: NOT integral
G3L &= ok
log(f"  the degree-3 witness: S1 = a sigma_1^3 + b sigma_1 sigma_2 + "
    f"c sigma_3 + e Delta has the UNIQUE rational solution")
log(f"    {dict(zip(('a', 'b', 'c', 'e'), [str(v) for v in xs]))}"
    f"  -> b = e = 1/2, NOT integral: S1 NOT in the module lattice "
    f"{'OK' if ok else 'MISMATCH'}")
xs2 = L3.solve(S.Matrix([vec_of_poly(pscale(S1P, 2), 3)]).T)
ok = all(e == int(e) for e in xs2)
G3L &= ok
log(f"  2 S1 IS in the module lattice : {'OK' if ok else 'MISMATCH'}")

# (d) the top-class quotient: H^6(B) = M_3 / <sigma_1^3, sigma_1 sigma_2,
#     sigma_3> = Z.<[S1]>, with [Delta] = 2 [S1] -------------------------
INV = [A1P, S1P, S2P, SIG3]
Binv = S.Matrix([vec_of_poly(p, 3) for p in INV]).T
rels = [ppow(SIG1, 3), pmul(SIG1, SIG2), SIG3]
Xrel = Binv.solve(S.Matrix([vec_of_poly(p, 3) for p in rels]).T)
G3L &= all(e == int(e) for e in Xrel)
rows = [[int(v) for v in Xrel[:, j]] for j in range(Xrel.cols)]
log(f"  relations in the invariant basis {{A1, S1, S2, sigma_3}}: {rows}")
Sn = smith_normal_form(S.Matrix(rows).T, domain=ZZ)
invs = [abs(int(Sn[i, i])) for i in range(min(Sn.shape)) if Sn[i, i] != 0]
free_rank = 4 - len(invs)
ok = (free_rank == 1 and all(v == 1 for v in invs))
G3L &= ok
log(f"  M_3/<sigma_1^3, sigma_1 sigma_2, sigma_3>: free rank {free_rank}, "
    f"invariant factors {invs} -> Z, no torsion : "
    f"{'OK' if ok else 'MISMATCH'}")
ok = (gcd(2, 3) == 1)
G3L &= ok
log("  the torsion relation 2 tau^3 = 0 kills the order-3 class tau^3 "
    "(gcd(2,3) = 1) : OK")
vD = [int(v) for v in Binv.solve(S.Matrix([vec_of_poly(DELTA, 3)]).T)]
vS1c = [int(v) for v in Binv.solve(S.Matrix([vec_of_poly(S1P, 3)]).T)]
t2 = [vD[i] - 2 * vS1c[i] for i in range(4)]
ok = in_rowlattice(t2, rows)
G3L &= ok
log(f"  Delta - 2 S1 lies in the relation lattice : "
    f"{'OK' if ok else 'MISMATCH'}")
t1 = [vD[i] - vS1c[i] for i in range(4)]
ok = (not in_rowlattice(t1, rows))
G3L &= ok
log(f"  Delta - S1 NOT in the relation lattice (so [Delta] != [S1]) : "
    f"{'OK' if ok else 'MISMATCH'}")
ok = (not in_rowlattice(vS1c, rows))
G3L &= ok
log(f"  S1 NOT in the relation lattice (so [S1] != 0, the quotient lives) : "
    f"{'OK' if ok else 'MISMATCH'}")
gate("G3L", G3L, "lattice-index layer: orbit-sum lattice, module indices "
     "(1,1,1,2,2,4,8,16,32), the S1 witness, H^6(B) = Z.<[S1]> with "
     "[Delta] = 2[S1]")
...
with open("/home/z/my-project/scripts/w31_derivation_check_output.txt", "w") as f:
    f.write("\n".join(OUT))
json.dump({"gates": GATES, "all": allpass,
          ...
          open("/home/z/my-project/scripts/w31_derivation_check.json", "w"),
          indent=1)
