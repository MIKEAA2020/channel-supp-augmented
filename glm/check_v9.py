#!/usr/bin/env python3
"""v9 manuscript consistency checker (inherits v8) — both papers.
...
PATHS = {
    "INST": "/home/z/my-project/channel-supp-augmented/manuscript uploads v9/instruments-paper-revised9.txt",
    "MAIN": "/home/z/my-project/channel-supp-augmented/manuscript uploads v9/main-article-revised9.txt",
}
FAIL = []
...
print(f"\n{'='*70}\nV9 ALIGNMENT CHECKS (wave 28)\n{'='*70}")
check("V9", "Flag-Quotiet" not in I, "no typo 'Flag-Quotiet' in INST title")
check("V9", "Flag-Quotient Width Obstructions" in I, "INST title updated")
check("V9", I.count("independently re-implemented") + I.count("independent re-implementation") + I.count("doubly implemented") >= 7, "INST re-implementation mentions >= 7 (all forms: abstract/intro/spec/lemma/remark/conclusion/lit-note)")
check("V9", "flag manifolds; equivariant cohomology" in I, "INST keywords extended")
check("V9", "constraint propagation" in I, "INST residuals paragraph updated (algorithms named)")
check("V9", "covering $p\\le 31$" in I, "INST prime-scan residual updated to p<=31")
check("V9", "the premise now rests on two independent implementations" in I, "INST open problem (vi) updated")
check("V9", "now doubly implemented" in I, "INST rem:flag-literature note updated")
check("V9", "the flat-width classification is complete" in I, "INST abstract gains the flat-width classification")
check("V9", "flat-width dichotomy" in M, "MAIN abstract gains the dichotomy clause")
check("V9", "flat-width dichotomy; flag-quotient cohomology" in M, "MAIN keywords extended")
check("V9", "machine-certified and independently re-implemented" in M, "MAIN sync clause updated")
check("V9", "not independently\\nre-implemented" not in I and "but not\\nindependently re-implemented" not in I
      and "but not independently re-implemented" not in I.replace("\n", " ").replace("\ ", " "),
      "the stale v8 residual phrase is gone")
mc = I[I.find('label{rem:machine-certificate}'):I.find('label{rem:machine-certificate}') + 3600]
check("V9", "column ratios" in mc and "incremental subgroup-order" in mc, "residuals name the fresh algorithms")
