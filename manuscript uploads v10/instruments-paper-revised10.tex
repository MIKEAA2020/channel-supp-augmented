...
coordinate flags $(j,k)$ and the constant $2(q-1)/q$), Theorem~\ref{thm:tverberg}
...
\]
In particular the flat value $1$ fails at $r=2$ already for $(n,d_B)\in\{(7,1),(2,4)\}$, and --- combining the concentration gate of Lemma~\ref{lem:slice-gate} with Theorem~\ref{thm:state-d2} --- for every instance with $d_B\ge3$ at any $n$: flatness throughout $2\le r\le nd_B^2-2$ is refuted at every output dimension, and the corrected statement is Conjecture~\ref{con:coord-flat}.
\end{corollary}
...
\begin{remark}[The machine certificate behind Lemma~\ref{lem:flag-cohomology}]
...
over the stratified base, seam-subdivided). The certification battery is
...
re-derived independently and stress-tested on two externally known model
cases: the lens space $L(3;1,1,1)$, where $x^2\ne0$ and $d_3^{1,2}=0$, and
...
computation agree. Residuals, stated plainly. On the hand side, the derivation of
...
and the full machine transcripts are available in the public companion
...
compression-width chain. The hand derivation of Proposition~\ref{prop:cupsquare} is the
...
$r=n$); the exact boundary values $4/3$ (Corollary~\ref{cor:exact-d1}); and the exact
...
at $n=2$) and $\delta_1\ge4/3$ on every other input-independent body; the two
one-outcome cases left bracketed in the previous version are closed in the negative
direction --- the qutrit state space has $\delta_1=4/3$ exactly and the ququart lies in
...
The flat direction is proved for $d_B\in\{1,2\}$ (Corollaries ~\ref{cor:classical-dichotomy} and~\ref{cor:qubit-dichotomy}) and fails at $r\in\{1,2\}$ for every $d_B\ge3$ (Theorems~\ref{thm:equal-basis} and~\ref{thm:state-nonflat}, together with Theorem~\ref{thm:state-d2}): the conjecture has been corrected twice, each time by a state-space result. The first form, without the clause $r\ge2$, predicted $\delta_1=1$ on the one-outcome bodies with $nd_B\le4$, i.e. exactly the two residual state-space cases, and is refuted --- the qutrit has $\delta_1=4/3$ exactly and the ququart at least $4/3$. The second form, with the clause $r\ge2$, predicted $\delta_2=1$ on the one-outcome qutrit ($nd_B=3\le2\cdot2+2$), and is refuted by Theorem~\ref{thm:state-d2} --- the qutrit has $\delta_2=4/3$ exactly --- which is why the clause now reads $r\ge3$. The content of the conjecture is therefore the flat direction at $d_B\ge3$ with $r\ge3$: that
...
[Authors anonymized for review], \emph{Exact diamond balls, antipodal widths, and query-uniform compression bounds for quantum channels}, submitted for publication (2026). (Submitted concurrently with the present work.)
