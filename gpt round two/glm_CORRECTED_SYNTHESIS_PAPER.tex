...
Three distinct radius-like quantities are used throughout this paper, always written with their qualifiers: (i) the \emph{centred relative in-radius} $\rho_{\diamond,\mathrm{inst}}(\mathcal I_0)$, the largest relative ball about a fixed centre that is contained in the body; (ii) the \emph{worst-case covering radius} $R_{\mathrm{inst}}(\Psi)$, the maximal distance from a centre to the body; and (iii) the \emph{antipodal profile} $\alpha_r$, a width-like sequence indexed by the sphere dimension $r$, which is not the radius of any ball. The word ``radius'' without a qualifier is avoided.
\end{remark}
...
\end{cases}
\]
When $L_{A,B}^{(n)}=D_{A,B}^{(n)}-1$ the middle range is empty (this is always the case for $d_A=1$). Every component of the envelope is rigorously certified: the value $1$ on the first range by Theorems~\ref{thm:replacement-profile} and \ref{thm:observable} and Corollary~\ref{cor:join-dimension}, the value $\gamma_{A,B}^{(n)}$ on the middle range by Theorems~\ref{thm:ball} and \ref{thm:full-direction}, and the value $0$ on the last range by Theorem~\ref{thm:zero-error}.
\end{definition}
