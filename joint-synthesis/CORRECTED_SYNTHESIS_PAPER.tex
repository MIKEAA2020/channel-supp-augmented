...
\]
and observable-sphere and orthogonal-join constructions extending the separation-$2$ regime to an explicit dimension $L_{A,B}^{(n)}$. These certificates combine into a certified lower envelope $\beta_r^{\mathrm{best},(n)}(A,B)$, and, together with certified upper bounds from constant codes, convex projection, and balanced input/output pinching, produce the explicit bracket
\[
...
U_r^{(n)}(A,B),
\]
with the intermediate widths $\delta_{r,\mathrm{inst}}^{\diamond}(A,B)$ left open between the two envelopes in general.
...
\]
A reference system of input dimension $d_A$ suffices for the diamond norm~\cite{Watrous2018}. The summands are the unconditional, generally subnormalised outcome contributions. The body $\Inst_n(A,B)$ is compact and convex: convexity follows from the convexity of the completely positive cone and the affine normalisation constraints, and compactness from positivity together with the summed marginal condition, which bounds every Choi block.
\end{definition}
...
The certified lower bounds rest on a single topological device, the antipodal profile of the body: an antipodally separated sphere inside the body forces every compression with that many latent dimensions to err. This section builds three such spheres --- the flagged replacement sphere, the full-direction sphere, and the observable sphere, the first and the third then combined by an instrument-level join --- and combines them into the certified lower envelope $\beta_r^{\mathrm{best},(n)}$.
...
\]
The construction is a continuous image of the sphere and may fail to be injective: $H$ and its positive scalar multiples define the same instrument. Injectivity is not required by the width arguments.
\end{theorem}
...
The bounds of Theorem~\ref{thm:pinching} are certified upper bounds on the compression width: they are achieved by the explicit pinching constructions above; the equalities $\delta_{r,\mathrm{inst}}^{\diamond}(A,B)=2(1-1/\ell_{\mathrm{out}})$ and $\delta_{r,\mathrm{inst}}^{\diamond}(A,B)=2(1-1/k_{\mathrm{in}})$ are not established at any $r$.
\end{remark}
...
\begin{tabular}{p{3.5cm}p{2.3cm}p{2.7cm}p{2.5cm}p{4.6cm}}
\toprule
