...
\label{rem:notation}
To avoid collisions, ranks are denoted $r_R$, the number of input pinching blocks by $k_{\mathrm{in}}$, the number of output pinching blocks by $\ell_{\mathrm{out}}$, the latent dimension by $r$ (or $q$ for general latent spaces), and the covering radius by $R$.
\end{remark}
...
\begin{proof}
If $\lambda_{\max}(-X)\le0$ there is nothing to prove. Otherwise, since $X$ is block diagonal, $\lambda_{\max}(-X)=\max_k\lambda_{\max}(-X_k)$. Choose $k_0$ attaining the maximum and a unit vector $|v\rangle\in A_0\otimes B$ with $X_{k_0}|v\rangle=-\lambda|v\rangle$, $\lambda=\lambda_{\max}(-X)$. Let $S\subseteq A_0$ be the support of $\Tr_B|v\rangle\langle v|$ and put $b=\dim S\le s$. If $|e_1\rangle,\dots,|e_b\rangle$ is an orthonormal basis of $S$ and $|\overline e_j\rangle\in A$ its coordinatewise conjugate under the fixed Choi bases, define
\[
|\omega_S\rangle=\frac1{\sqrt b}\sum_{j=1}^b|e_j\rangle_{A_0}|\overline e_j\rangle_A.
\]
With $P_S$ the projection onto $S$, the normalised-Choi convention gives
\[
Y:=(\id_{A_0}\otimes\widehat{\vD})(|\omega_S\rangle\langle\omega_S|)
=\frac{d_A}{b}(P_S\otimes I_{B\otimes C_n})X(P_S\otimes I_{B\otimes C_n}).
\]
The direction constraint implies
\[
\Tr Y=\frac{d_A}{b}\Tr\Big[P_S\sum_{k=1}^n\Tr_BX_k\Big]=0,
\]
so $Y$ is Hermitian and traceless. Since $|v\rangle\in S\otimes B$, testing on $|v,k_0\rangle:=|v\rangle\otimes|k_0\rangle$ gives
\[
...
\]
The state $|\omega_S\rangle\langle\omega_S|$ is admissible in the instrument diamond-norm optimisation, so $\|\vD\|_{\diamond,\mathrm{inst}}\ge\|Y\|_1$, and rearranging proves the claim.
\end{proof}
...
\emph{Case $d_B>1$.} Choose an $s$-dimensional input subspace $S_A\subseteq A$ and two isometries $V_0,V_1:S_A\to B$ with $\Tr(V_0^\dagger V_1)=0$ (if $s=1$, two orthogonal output unit vectors; if $s\ge2$, fix $V_0$, take a trace-zero unitary $U$ on $S_A$, e.g.\ diagonal with roots of unity, and set $V_1=V_0U$). With $P_{S_A}$ the input projection define the completely positive, trace-nonincreasing maps
\[
\mathcal E_j(Z)=V_jP_{S_A}ZP_{S_A}V_j^\dagger,\qquad j=0,1.
\]
Choose an orthonormal basis $|e_1\rangle,\dots,|e_s\rangle$ of $S_A$ and set
\[
|\omega_{S_A}\rangle=\frac1{\sqrt s}\sum_{a=1}^s|\overline e_a\rangle_{A_0}|e_a\rangle_A,
\qquad
P_j=(\id\otimes\mathcal E_j)(|\omega_{S_A}\rangle\langle\omega_{S_A}|).
\]
The $P_j$ are rank-one projectors with
\[
\Tr_BP_0=\Tr_BP_1=\frac{P_{\overline S_A}}s,
\qquad
...
\]
Since $\Tr\mathcal E_1(Z)=\Tr(P_{S_A}Z)=\Tr\mathcal E_0(Z)$, the summed direction is trace-annihilating, so $\vD\in\mathfrak V_{A,B}^{(n)}$; its flagged diamond norm is $1$ and its first normalised Choi block is $\Jhat_{\Delta_1}=\tfrac{s}{2d_A}(P_1-P_0)$. At $t=2/(nd_Bs)$ the first block of $\mathcal I_*+t\vD$ equals
\[
...
\[
\zeta_R=\Tr_A[(I_R\otimes P)\xi(I_R\otimes P)]=\Tr_A[(I_R\otimes P)\xi]\ge0.
\]
Then
\[
(\id_R\otimes\widehat{\vD})(\xi)
=\frac12\,\zeta_R\otimes\big(|2\rangle\langle2|-|1\rangle\langle1|\big),
\]
whose trace norm is $\Tr\zeta_R\le1$, with equality attained by an input supported on $P$; hence $\|\vD\|_{\diamond,\mathrm{inst}}=1$. In the fixed Choi basis, $\Jhat_{\mathcal F}=P^{\mathsf T}/d_A$. At $t=2/n$ the first two Choi blocks of $\mathcal I_*+t\vD$ are
\[
...
Let $\sigma_{RB}\ge0$ be subnormalised, with $\rho_R=\Tr_B\sigma_{RB}$ and $r_R=\rank\rho_R$. Then
\[
\sigma_{RB}\le\min\{r_R,d_B\}\,\rho_R\otimes I_B.
\]
\end{lemma}

\begin{proof}
If $\rho_R=0$ there is nothing to prove. Otherwise let $R_0=\supp\rho_R$, so that $\rho_R^{-1/2}$ is defined on $R_0$ (extended by zero on the orthocomplement), and set
\[
Z=(\rho_R^{-1/2}\otimes I_B)\sigma_{RB}(\rho_R^{-1/2}\otimes I_B).
\]
Then $Z\ge0$ and $\Tr_BZ=I_{R_0}$. Write $Z=\sum_\alpha|K_\alpha\rangle\!\rangle\langle\!\langle K_\alpha|$ with $\sum_\alpha K_\alpha^\dagger K_\alpha=I_{R_0}$ in the vectorisation convention $|K\rangle\!\rangle=\sum_i|\overline i\rangle\otimes K|i\rangle$. For a unit vector $|v\rangle\in R_0\otimes B$ let $V:R_0\to B$ be the corresponding matrix of rank $b\le\min\{r_R,d_B\}$. Using $|\Tr M|^2\le\rank(M)\|M\|_2^2$ with $M=K_\alpha V^\dagger$ of rank at most $b$,
\[
\langle v|Z|v\rangle
=\sum_\alpha|\Tr(V^\dagger K_\alpha)|^2
\le b\sum_\alpha\Tr(VK_\alpha^\dagger K_\alpha V^\dagger)=b.
\]
Thus $Z\le\min\{r_R,d_B\}I_{R_0\otimes B}$, and conjugation by $\rho_R^{1/2}\otimes I_B$ proves the claim.
\end{proof}
...
\[
\sigma_{RBC_n}=\sum_{k=1}^n\sigma_{k,RB}\otimes|k\rangle\langle k|,\qquad
\sum_k\Tr_B\sigma_{k,RB}=\rho_R.
\]
Put $\rho_{k,R}=\Tr_B\sigma_{k,RB}$. Since $\rank\rho_{k,R}\le d_A$, Lemma~\ref{lem:domination} gives
\[
\sigma_{k,RB}\le s\,\rho_{k,R}\otimes I_B\le s\,\rho_R\otimes I_B.
\]
Therefore
\[
\sigma_{RBC_n}\le nd_Bs\,\Big(\rho_R\otimes\frac{I_B}{d_B}\otimes\frac{I_{C_n}}n\Big),
\]
...
For equality, choose an $s$-dimensional input subspace $S\subseteq A$, an isometry $V:S\to B$, and a state $\omega$ on $B$. With $P_S$ the subspace projection define the channel
\[
\Phi(X)=VP_SXP_SV^\dagger+\Tr[(I_A-P_S)X]\,\omega,
\]
whose action on the selected Schmidt-rank-$s$ subspace is a pure isometry while its action outside $S$ is a fixed replacement (in particular, no claim of extremality is made or needed). Set $\mathcal I_1=\Phi$ and $\mathcal I_k=0$ for $k\ge2$. A maximally entangled input of Schmidt rank $s$ on $S$ gives a pure output in the first flag, whereas $\mathcal I_*$ gives the maximally mixed state on the corresponding $nd_Bs$-dimensional support; the trace norm of their difference is $2(1-1/(nd_Bs))$ (equivalently, the trace distance is $1-1/(nd_Bs)$). Hence $R_{\mathrm{inst}}(\mathcal I_*)\ge2(1-1/(nd_Bs))$, and equality holds.
...
For the replacement family above this range, compose any map $S^r\to\RepInst_n(A,B)$ with the inverse isometry and affine coordinates in $\mathbb R^{m}$. For $r=m$ Borsuk--Ulam applied to $S^{m}\to\RepInst_n(A,B)\to\mathbb R^{m}$ produces an antipodal collision; for $r>m$, restrict the map to an equatorial $S^m\subset S^r$ first. Every such map therefore has minimum antipodal distance zero, proving the second part of the formula.
\end{proof}
...
\]
If two block sizes satisfy $a\ge b+2$, replacing them by $a-1$ and $b+1$ changes their squared sum by $-2(a-b-1)<0$; iteration shows that balanced blocks minimise $\sum_jb_j^2$ at fixed $\ell$.
...
Covering radius $R(\mathcal I_*)$ & $2(1-1/d_Bs)$ & $2(1-1/nd_Bs)$ & $\sigma_{RBC_n}\le nd_Bs\,(\rho_R\otimes I_B/d_B\otimes I_{C_n}/n)$ plus $a\le Kb$\\
