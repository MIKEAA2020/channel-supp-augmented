...
\newcommand{\Herm_0}{\mathrm{Herm}_0}
...
\]
with $\mathcal I_*$ an optimal centre and the diameter of the body exactly $2$; the same value is also the \emph{global} in-radius: no other centre of the body carries a larger relative ball. The flagged replacement body has exact antipodal profile
\[
...
U_r^{(n)}(A,B),
\]
with the intermediate widths $\delta_{r,\mathrm{inst}}^{\diamond}(A,B)$ not claimed to be exactly determined in general.
...
\]
A reference system of input dimension $d_A$ suffices for the diamond norm~\cite{Watrous2018}. The summands are the unconditional, generally subnormalised outcome contributions; no worst conditional-branch normalisation is used. The body $\Inst_n(A,B)$ is compact and convex: convexity follows from the convexity of the completely positive cone and the affine normalisation constraints, and compactness from positivity together with the summed marginal condition, which bounds every Choi block.
\end{definition}
...
\]
i.e.\ the advantage over random guessing is $\tfrac14\|\mathcal I-\mathcal J\|_{\diamond,\mathrm{inst}}$ (a standard instance of the diamond norm's role in channel discrimination~\cite{Watrous2018}). Thus every radius and width stated in the instrument diamond norm has a direct single-shot discrimination interpretation, up to this conventional factor of $1/4$.
\end{remark}
...
Theorem~\ref{thm:exact-radius} determines the largest ball centred at $\mathcal I_*$. The global in-radius
\[
\sup_{\mathcal I_0\in\Inst_n(A,B)}\rho_{\diamond,\mathrm{inst}}(\mathcal I_0)
\]
is determined by Theorem~\ref{thm:global-inradius} (after Theorem~\ref{thm:covering} below): the in-radius is a concave function of the centre, so the same symmetrisation that makes $\mathcal I_*$ optimal for the covering radius makes it optimal for the in-radius, and the supremum equals $\rho_n=2/(nd_Bs)$. In particular no re-centring of the radial construction below can certify more than $\rho_n$ from a centred ball.
\end{remark}
...
$\rho_n=2/(nd_Bs)$ certifies a centrally symmetric norm ball in the full direction space $\mathfrak V_{A,B}^{(n)}$, while the sphere $S^{nd_B^2-2}$ via $X\mapsto X_+/\Tr X_+$ certifies a non-linear, antipodally $2$-separated sphere in the flagged replacement body. There is no contradiction between $\rho_n<1$ and $\alpha_r=1$ for $r\le nd_B^2-2$: these are different geometric mechanisms (a ball radius versus an antipodal profile) acting on different subsets of the body. The ball bound governs the intermediate regime, while the replacement sphere governs the small-index regime.
\end{remark}
...
J^-(Z)=\lambda\Theta_-+(1-\lambda)\nu\otimes\tau_0.
\]
Both $J^\pm(Z)$ are positive and flag-block diagonal --- the $\Theta_\pm$ summands inherit flag-block diagonality from $Z$ through the Jordan decomposition, and the filler $\nu\otimes\tau_0$ is flag-block diagonal because $\tau_0$ is diagonal in the fixed flag basis --- with $A_0$-marginal
\[
\lambda\mu+(1-\lambda)\nu=\frac{I_{A_0}}{d_A},
\]
...
\]
The map is a continuous image of the sphere; no injectivity is asserted.
\end{theorem}
...
\begin{proof}
If $r=D_{A,B}^{(n)}$, affine coordinates give exact reconstruction. Assume henceforth $0\le r<D_{A,B}^{(n)}$, and put $k=D_{A,B}^{(n)}-r\ge1$. Project affine coordinates of the compact convex instrument body onto $\mathbb R^r$; the linear coordinate projection is continuous in finite dimensions. Each nonempty fibre $F$ is compact and convex, has diameter at most two (Proposition~\ref{prop:diameter}), and has affine dimension at most $k$. Equip $\aff(F)$ with the restriction of the instrument diamond norm, and let
\[
\overline B_{\diamond,\mathrm{inst}}(x,R)
=\big\{z\in\aff(F):\|z-x\|_{\diamond,\mathrm{inst}}\le R\big\}
\]
be the closed ball in that norm; these are compact convex sets, since every norm ball is convex. Set $R=2k/(k+1)$ and consider the family $\{F\}\cup\{\overline B_{\diamond,\mathrm{inst}}(x,R):x\in F\}$ in $\aff(F)\simeq\mathbb R^k$. Any subfamily of at most $k+1$ members intersects: if it contains $m$ balls centred at $x_1,\dots,x_m$, their barycentre $c=\tfrac1m\sum_jx_j$ lies in $F$ by convexity, and pairwise distances in the fibre are at most $2$, so
\[
\|c-x_j\|_{\diamond,\mathrm{inst}}
\le\frac{2(m-1)}m\le\frac{2k}{k+1}=R
\]
(the second inequality is equivalent to $m\le k+1$; if $F$ is also a member of the subfamily then $m\le k$, which is even stronger). Helly's theorem~\cite{Barvinok2002} and compactness of $F$ therefore give a point of $F$ within distance $R$ of the entire fibre. Use these points as decoder values.
\end{proof}

\begin{remark}[Nonconstructive Helly decoder]
\label{rem:nonconstructive}
The decoder in Proposition~\ref{prop:convex} is selected via Helly's theorem and compactness for each fibre; it is guaranteed to take values in the fibre and to achieve error $\le2k/(k+1)$, but its continuity is not claimed and is not required by Definition~\ref{def:width}. The word ``certified'' refers to proved upper bounds, not to continuous or algorithmic decoders. If both encoder and decoder were required to be continuous, the same zero-error threshold $r\ge D_{A,B}^{(n)}$ remains valid: the upper direction is Theorem~\ref{thm:zero-error} (which supplies a continuous decoder), and the Borsuk--Ulam lower bound applies a fortiori to continuous decoders. The subcritical upper bounds, however, become existence statements whose Helly-selected decoders may fail continuity.
\end{remark}
...
Two candidate tools, however, do \emph{not} break the barrier, and it is worth recording why. (i) \emph{Covering-number encodings.} If the body contains a diamond ball of radius $r_0$ (which exists by full affine dimension and norm equivalence), then an $\varepsilon$-net of the body in the instrument diamond norm has size $N(\varepsilon)\ge(r_0/\varepsilon)^{D}$ by the volume estimate, so a net-based code needs at least $D\log_2(r_0/\varepsilon)$ bits, which exceeds $D$ bits for every $\varepsilon<r_0/2$. The affine encoding achieves error $0$ with $D$ Euclidean coordinates, and Theorem~\ref{thm:zero-error} shows that $D$ coordinates are also necessary among continuous encoders; this comparison with discrete net-based codes is heuristic, is not used in any proof, and does not by itself rule out other continuous low-dimensional encodings. (ii) \emph{Orthogonal joins of full-body spheres.} These are precisely the constructions already absorbed into $\beta_r^{\mathrm{best},(n)}$ in Section~\ref{sec:lower}; they certify the value $1$ up to $L_{A,B}^{(n)}$ and cannot extend the separation-two regime beyond it.
\end{remark}
...
\]
with covering radius $2(1-1/(d_B\min\{d_A,d_B\}))$ and zero-error threshold $d_A^2(d_B^2-1)$. For $d_B=1$ the channel body is the singleton trace functional of Remark~\ref{rem:degenerate}. (All normalisations are the ones fixed in Remark~\ref{rem:choi-convention}; no comparison with external conventions is asserted.)
